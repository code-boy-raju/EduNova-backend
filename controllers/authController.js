...
const studentSignup = async (req, res) => {
  ...
  try {
    ...
    await sendEmail(
      email,
      "Verify Your Student Account - OTP",
  ...
  try {
    ...
    await sendEmail(
      ...
      `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: #f9f9f9; border: 1px solid #ddd; border-radius: 8px;">
        <h2 style="color: #191818ff; text-align: center;">Approve or Reject Instructor</h2>
        <p><strong>${username}</strong> has applied to become an instructor.</p>
    ...
    await userModel.create(pendingInstructor);
    res.status(200).json({
      message: "✅ Email approval sent to admin. In case of approval, you will get a success email."
    });
...
const verifyInstructor = async (req, res) => {
  ...
  try {
    ...
    if (status === "approve") {
      ...
      await sendEmail(
        email,
        "Instructor Approval Accepted",
        `<h2>✅ Your instructor account has been approved!</h2>
         <p>You can now log in to your instructor dashboard.</p>`
      );
    ...
    if (status === "reject") {
      ...
      if (user) {
        ...
        await sendEmail(
          email,
          "Instructor Approval Rejected",
          `<h2>❌ Sorry, your instructor application has been rejected.</h2>
           <p>You may reapply later.</p>`
        );
      }
